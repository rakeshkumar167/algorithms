...
public class Knapsack01BoundedTopDownApproach {

	public static void main(String[] args) {
		int[] weightArray = new int[] { 2, 4, 5, 3, 10, 1 };
		int[] valueArray = new int[] { 11, 4, 7, 16, 5, 17 };
		int maxWeight = 7;
	...
	private static int getMaxValueKnapsack(int[] weightArray, int[] valueArray, int capacity, int index, int[][] dp) {
		...
		if (capacity >= weightArray[index]) {
			val1 = valueArray[index] + getMaxValueKnapsack(weightArray, valueArray, capacity - weightArray[index], index + 1, dp);
		}
